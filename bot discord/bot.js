...
async function generateChallengeImage(db) {
  ...
  const htmlContent = `
    ...
    <head>
      <style>
        ...
        .header h1 { font-size: 56px; color: #d4af37; text-transform: uppercase; letter-spacing: 14px; margin: 0; font-weight: 900; text-shadow: 0 0 30px rgba(212,175,55,0.6); }
        .header p { color: rgba(255,255,255,0.6); margin-top: 15px; font-size: 22px; text-transform: uppercase; letter-spacing: 6px; }
        .main-layout { display: grid; grid-template-columns: 1fr 400px; gap: 50px; }
        .card { 
        ...
        .icon { width: 95px; height: 95px; border-radius: 14px; border: 2px solid rgba(212,175,55,0.3); }
        .details { display: flex; flex-direction: column; gap: 8px; }
        .name { font-size: 32px; font-weight: 900; color: #fff; margin: 0; letter-spacing: -0.5px; }
        .rarity { font-size: 14px; font-weight: 900; text-transform: uppercase; padding: 3px 10px; background: rgba(255,255,255,0.1); border-radius: 6px; color: #d4af37; width: fit-content; }
        .desc { font-size: 20px; color: rgba(255,255,255,0.9); line-height: 1.4; margin: 0; font-weight: 500; }
        .reward { font-size: 34px; font-weight: 900; color: #f1c40f; text-shadow: 0 0 15px rgba(241,196,15,0.4); text-align: right; }
        
        .top-hunters { background: rgba(212, 175, 55, 0.08); border: 2px solid rgba(212, 175, 55, 0.25); border-radius: 24px; padding: 35px; height: fit-content; }
        .hunter-title { font-size: 24px; font-weight: 900; color: #d4af37; text-align: center; margin-bottom: 30px; letter-spacing: 3px; border-bottom: 2px solid rgba(212, 175, 55, 0.2); padding-bottom: 20px; }
        .hunter-row { display: flex; align-items: center; padding: 18px 0; border-bottom: 1px solid rgba(255,255,255,0.08); }
        .hunter-rank { font-size: 24px; font-weight: 900; color: #d4af37; width: 45px; }
        .hunter-name { flex: 1; font-size: 22px; font-weight: 700; color: #fff; }
        .hunter-count { font-size: 22px; font-weight: 900; color: #f1c40f; }
        .no-hunters { text-align: center; opacity: 0.6; padding: 30px; font-style: italic; font-size: 18px; }

        .footer { text-align: center; margin-top: 60px; font-size: 16px; color: rgba(255,255,255,0.3); text-transform: uppercase; letter-spacing: 8px; font-weight: 900; }
      </style>
    </head>
    <body>
      <div class="content">
  ...
  try {
    const page = await browser.newPage();
    await page.setViewport({ width: 1250, height: 100, deviceScaleFactor: 2 }); 
    await page.setContent(htmlContent, { waitUntil: 'networkidle0', timeout: 30000 });
    const bodyHeight = await page.evaluate(() => document.body.scrollHeight);
    await page.setViewport({ width: 1250, height: bodyHeight, deviceScaleFactor: 2 });
